...
const rejectSchema = z.object({ comments: z.string().min(1, "A reason is required") });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (current.status !== "SUBMITTED") throw new ApiError("Only a submitted request can be rejected", 409);
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const request_ = await tx.priceApprovalRequest.update({
        where: { id },
        data: { status: "REJECTED", reviewedById: user.id, reviewedAt: new Date(), reviewComments: input.comments }
      });
      await tx.notification.create({
        data: {
          userId: current.requestedById,
          title: "Pricing request rejected",
          message: `${request_.requestNumber} was rejected: ${input.comments}`,
          type: "PRICE_APPROVAL",
